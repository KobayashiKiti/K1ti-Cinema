...
function displayMovies(movies, containerId) {
    ...
    movies.forEach(movie => {
        ...
        movieElement.innerHTML = `
            ...
            <button class="book-ticket">Mua vé</button>
        `;
...
document.getElementById('show-now-showing').addEventListener('click', function() {
    document.getElementById('now-showing-section').style.display = 'flex';
...
displayMovies(nowShowingMovies, 'now-showing-section');
displayMovies(comingSoonMovies, 'coming-soon-section');
...
document.getElementById('ticketForm').addEventListener('submit', function(event) {
    ...
    if (date && time && cinema && selectedSeats.length > 0) {
        window.location.href = "pages/member.html"; // Redirect to member page
    } else {
